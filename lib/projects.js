...
export async function getProjectBySlug(moduleId, slug) {
    const snapshot = await get(dbRef(rtdb, `${getFirebasePath(moduleId, 'projects')}/${slug}`));
...
export async function saveProject(moduleId, form, options = {}) {
    ...
    await set(dbRef(rtdb, `${getFirebasePath(moduleId, 'projects')}/${targetSlug}`), payload);

    // ── If slug changed during update, remove old entry ──────────────────────
    if (originalSlug && originalSlug !== targetSlug) {
        await remove(dbRef(rtdb, `${getFirebasePath(moduleId, 'projects')}/${originalSlug}`));
    }
...
export async function deleteProject(moduleId, slug) {
    ...
    await remove(dbRef(rtdb, `${getFirebasePath(moduleId, 'projects')}/${slug}`));
}
...
export function subscribeToProjectEntries(moduleId, onEntries, onError) {
    return onValue(
        dbRef(rtdb, `${getFirebasePath(moduleId, 'projects')}`),
        (snapshot) => {
