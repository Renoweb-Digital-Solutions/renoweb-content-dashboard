...
export async function getCaseStudyById(moduleId, id) {
    const snapshot = await get(dbRef(rtdb, `${getFirebasePath(moduleId, 'case-studies')}/${id}`));
...
export async function saveCaseStudy(moduleId, form, options = {}) {
    ...
    await set(dbRef(rtdb, `${getFirebasePath(moduleId, 'case-studies')}/${targetId}`), payload);

    if (originalId && originalId !== targetId) {
        await remove(dbRef(rtdb, `${getFirebasePath(moduleId, 'case-studies')}/${originalId}`));
    }
...
export async function deleteCaseStudy(moduleId, id) {
    ...
    const entrySnapshot = await get(dbRef(rtdb, `${getFirebasePath(moduleId, 'case-studies')}/${id}`));
    ...
    await remove(dbRef(rtdb, `${getFirebasePath(moduleId, 'case-studies')}/${id}`));
}

export function subscribeToCaseStudies(moduleId, onEntries, onError) {
    return onValue(
        dbRef(rtdb, `${getFirebasePath(moduleId, 'case-studies')}`),
        (snapshot) => {
