...
export async function deleteCaseStudy(id) {
    ...
    const entrySnapshot = await get(dbRef(rtdb, `case-studies/${id}`));
    ...
    if (entry) {
        ...
        const addPath = (url) => {
            if (url) {
                const match = url.match(/\/contentimages\/([^"'\s]+)/);
                if (match && match[1]) {
        ...
        if (pathsToDelete.length > 0) {
            ...
            if (error) {
                console.error("Failed to delete case study images from Supabase:", error);
            }
        }
    }
