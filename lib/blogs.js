...
export async function getBlogBySlug(moduleId, slug) {
    const snapshot = await get(dbRef(rtdb, `${getFirebasePath(moduleId, 'blogs')}/${slug}`));
    return snapshot.exists() ? snapshot.val() : null;
}

// ── Save / Update ────────────────────────────────────────────────────────────

export async function saveBlog(moduleId, form, options = {}) {
    const {
        originalSlug = null,
        confirmOverwrite,
    } = options;
    ...
    const existing = await getBlogBySlug(moduleId, targetSlug);
    ...
    if (form.bannerFile) {
        ...
        bannerUrl = await uploadFile(`blog-banners/${moduleId}/${targetSlug}.${ext}`, form.bannerFile);
    }
    ...
    for (const match of blobMatches) {
        ...
        try {
            ...
            const fileName = `blog-images/${moduleId}/${targetSlug}/${Date.now()}-${Math.floor(Math.random() * 1000)}.${ext}`;
    ...
    if (originalSlug) {
        const originalEntry = originalSlug === targetSlug ? existing : await getBlogBySlug(moduleId, originalSlug);
    ...
    await set(dbRef(rtdb, `${getFirebasePath(moduleId, 'blogs')}/${targetSlug}`), payload);

    // ── If slug changed during update, remove old entry ──────────────────────
    if (originalSlug && originalSlug !== targetSlug) {
        await remove(dbRef(rtdb, `${getFirebasePath(moduleId, 'blogs')}/${originalSlug}`));
    }
...
export async function deleteBlog(moduleId, slug) {
    // 1. Fetch the entry to get the content and banner URL
    const entry = await getBlogBySlug(moduleId, slug);
    ...
    await remove(dbRef(rtdb, `${getFirebasePath(moduleId, 'blogs')}/${slug}`));
}

// ── Real-time Subscription ───────────────────────────────────────────────────

export function subscribeToBlogEntries(moduleId, onEntries, onError) {
    return onValue(
        dbRef(rtdb, `${getFirebasePath(moduleId, 'blogs')}`),
        (snapshot) => {
