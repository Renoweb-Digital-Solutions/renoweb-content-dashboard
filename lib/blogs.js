...
export async function getBlogBySlug(moduleId, slug) {
    const snapshot = await get(dbRef(rtdb, `${moduleId}/blogs/${slug}`));
...
export async function saveBlog(moduleId, form, options = {}) {
    ...
    await set(dbRef(rtdb, `${moduleId}/blogs/${targetSlug}`), payload);

    // ── If slug changed during update, remove old entry ──────────────────────
    if (originalSlug && originalSlug !== targetSlug) {
        await remove(dbRef(rtdb, `${moduleId}/blogs/${originalSlug}`));
    }
...
export async function deleteBlog(moduleId, slug) {
    ...
    await remove(dbRef(rtdb, `${moduleId}/blogs/${slug}`));
}
