...
export async function getPressById(moduleId, id) {
    const snapshot = await get(dbRef(rtdb, `${moduleId}/press/${id}`));
...
export async function savePress(moduleId, form, originalId = null) {
    ...
    if (payload.isFeatured) {
        ...
        const snapshot = await get(dbRef(rtdb, `${moduleId}/press`));
        if (snapshot.exists()) {
            const updates = {};
            const data = snapshot.val();
            for (const key in data) {
                if (data[key].isFeatured && key !== payload.id) {
                    updates[`${moduleId}/press/${key}/isFeatured`] = false;
                }
            }
    ...
    await set(dbRef(rtdb, `${moduleId}/press/${payload.id}`), payload);

    if (originalId && originalId !== targetId) {
        await remove(dbRef(rtdb, `${moduleId}/press/${originalId}`));
    }
...
export async function deletePress(moduleId, id) {
    ...
    await remove(dbRef(rtdb, `${moduleId}/press/${id}`));
}
...
export async function toggleFeaturedPress(moduleId, id, currentlyFeatured) {
    ...
    updates[`${moduleId}/press/${id}/isFeatured`] = !currentlyFeatured;
