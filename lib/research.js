...
export async function getResearchBySlug(moduleId, slug) {
    const snapshot = await get(dbRef(rtdb, `${getFirebasePath(moduleId, 'research-hub')}/${slug}`));
...
export async function saveResearch(moduleId, form, options = {}) {
    ...
    await set(dbRef(rtdb, `${getFirebasePath(moduleId, 'research-hub')}/${targetSlug}`), payload);

    // ── If slug changed during update, remove old entry ──────────────────────
    if (originalSlug && originalSlug !== targetSlug) {
        await remove(dbRef(rtdb, `${getFirebasePath(moduleId, 'research-hub')}/${originalSlug}`));
    }
...
export async function deleteResearch(moduleId, slug) {
    ...
    await remove(dbRef(rtdb, `${getFirebasePath(moduleId, 'research-hub')}/${slug}`));
}
...
export function subscribeToResearchEntries(moduleId, onEntries, onError) {
    return onValue(
        dbRef(rtdb, `${getFirebasePath(moduleId, 'research-hub')}`),
        (snapshot) => {
