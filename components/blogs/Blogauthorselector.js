...
import { AUTHORS } from "./Blogconstants";

// components/cms/blog/BlogAuthorSelector.js
// ─────────────────────────────────────────────────────────────────────────────
// Primary author + optional co-author selector.
// Mirrors the AuthorSelector in the Case Study form; uses the blog-local AUTHORS
// list from blogConstants so both can evolve independently.
// ─────────────────────────────────────────────────────────────────────────────
...
function AuthorCard({ author, selected, onSelect }) {
    ...
    return (
        ...
        >
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-600/30 to-blue-900/40 border-2 border-gray-800 flex items-center justify-center text-xs font-bold text-blue-400 flex-shrink-0">
                {author.image ? (
...
export default function BlogAuthorSelector({
    primaryAuthor,
    coAuthor,
    onPrimaryChange,
    onCoAuthorChange,
        ...
        <div className="space-y-5">
            ...
            <div>
                <label className="block text-xs text-gray-400 mb-2 font-medium">
                    Co-Author <span className="text-gray-600">(optional)</span>
                </label>
