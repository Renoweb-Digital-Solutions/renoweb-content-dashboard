...
// components/cms/research-hub/RHjsonmodal.js
// ─────────────────────────────────────────────────────────────────────────────
// Modal that renders the current research hub form as formatted JSON.
// Strips binary/base64 fields before display. Amber/orange accent.
// ─────────────────────────────────────────────────────────────────────────────

import { useState } from "react";

export default function RHJsonModal({ data, onClose }) {
    const [copied, setCopied] = useState(false);

    const exportData = {
        ...data,
        bannerFile: data.bannerFile ? "[FILE_OBJECT — will be uploaded to Firebase Storage]" : null,
            ...
            <div className="bg-black border border-gray-800 rounded-2xl w-full max-w-3xl max-h-[80vh] flex flex-col shadow-2xl">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="font-bold text-white text-sm">Research Hub JSON Output</h3>
                    </div>
                ...
                {/* Code */}
                <pre className="p-6 overflow-auto flex-1 text-xs text-amber-300/80 font-mono leading-relaxed bg-gray-950 rounded-none">
                    {json}
                </pre>

                {/* Footer */}
                <div className="px-6 py-4 border-t border-gray-800 flex items-center justify-between gap-3">
                    <p className="text-[10px] text-gray-600">Paste this object into your research hub data file</p>
