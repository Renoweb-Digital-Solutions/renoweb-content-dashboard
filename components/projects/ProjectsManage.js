            ...
            <div className="my-4 w-full max-w-4xl rounded-2xl border border-gray-800 bg-black shadow-2xl">
                <div className="flex items-center justify-between border-b border-gray-800 px-6 py-5">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-sm font-bold text-white">Update Project</h3>
...
function EntryCard({ entry, onDelete, onEdit }) {
    const coverImage = entry.images && Array.isArray(entry.images) ? entry.images.find(img => img !== null) : null;
        ...
        <div className="group overflow-hidden rounded-2xl border border-gray-800 bg-gray-900/50 transition hover:border-gray-700">
            <div className="relative h-36 bg-gradient-to-br from-blue-950/40 via-gray-900 to-cyan-950/20">
                {coverImage ? (
                    ...
                    <img src={coverImage} alt={entry.title} className="h-full w-full object-cover" />
                ) : (
        ...
        <div>
            ...
            <div className="mb-8 flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className="mt-0.5 text-xs text-gray-500">
                        {entries.length} projects · {featuredCount} featured
                    </p>
                </div>
            ...
            {(search || category || featuredOnly) && (
                <p className="mb-4 text-xs text-gray-600">
                    Showing {filteredEntries.length} of {entries.length} projects
                    <button
                        onClick={() => {
                            setSearch("");
                            setCategory("");
